...
const HOME_TITLE = "Which flower are you? | Flowering";

function setIndexing(on) {
  var robots = document.getElementById("meta-robots");
  if (robots) robots.setAttribute("content", on ? "index, follow" : "noindex, nofollow");
}
...
function renderResult(name, date) {
  ...
  lastShare = { name: name, date: date, flower: flower };
  writeResultUrl(name, date);
  document.getElementById("again").textContent = viewingShared ? "Find your flower" : "Try again";
  document.title = firstName(name) + " is " + articleFor(flower.name) + flower.name + " | Flowering";
  setIndexing(false);
...
function showForm() {
  ...
  viewingShared = false;
  clearResultUrl();
  document.getElementById("again").textContent = "Try again";
...
function writeResultUrl(name, date) {
  if (!history.replaceState) return;
  history.replaceState(null, "", resultUrl(name, date));
}
...
function readShareParams() {
  ...
  const name = (q.get("n") || "").trim().slice(0, 80);
  const date = q.get("d") || "";
  if (!name || !/^\d{4}-\d{2}-\d{2}$/.test(date)) return null;
  if (validate(name, date)) return null;
...
function copyResultLink() {
  if (!lastShare) return;
  const url = resultUrl(lastShare.name, lastShare.date);
  const n = firstName(lastShare.name);
  copyText(url, "Link copied. Anyone with it can see " + n + " as a flower.");
}

document.getElementById("birthdate").max = todayISO();

(function openSharedBloom() {
  const shared = readShareParams();
  if (!shared) return;
  viewingShared = true;
  renderResult(shared.name, shared.date);
})();
...
(function selfCheck() {
  ...
  const link = resultUrl("Zainab", "2000-04-01");
  console.assert(link.indexOf("n=Zainab") !== -1 && link.indexOf("d=2000-04-01") !== -1, "share link must carry name and date");
})();
